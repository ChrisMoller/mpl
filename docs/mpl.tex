...
%\font\tenrm= cmbx10 scaled \magstep 2
\textfont0=\vbigbx
...
J, at least, avoids all that, but has its own weirdnesses---it's not at all
intuitive that ``1!:1 $<$'abc.q''' means ``read file abc.q''---and, though it
may be a quibble on my part, the J default installation process and location
is, uh, ``non-standard..''

So, a few notes on {\bigsl MPL}...

First,  {\bigsl MPL} behaves a lot like  {\bigsl APL}---it reads from right to
left: the order of operation of
$$a * b - c$$
is
$$a * (b - c)$$
rather than the
